    ...
    if(city !='' && days != '')
        {
            $.ajax({
                ...
                url:'http://api.openweathermap.org/data/2.5/forecast/daily?q=' + city + "&units=metric" +"&cnt=" + days + "&APPID=3dc9a29630ee9a892dcd672335bf6e6b",
                 type: "GET" ,
              dataType: "jsonp",
              success : function(data){
                  var table='';
                  var header='<h2 style="font-weight:bold;font-size:30px;margin-top:20px;">Weather Forecast For City ' +data.city.name+',' +data.city.country+ '</h2>'
